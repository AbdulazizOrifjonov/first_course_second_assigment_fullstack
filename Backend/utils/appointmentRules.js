...
const MIN_APPOINTMENT_LEAD_MS = (30 * 60 + 30) * 60 * 1000;
...
function getEarliestAppointmentTime() {
...
function isAppointmentSlotAllowed(dateStr, timeStr) {
  if (!dateStr || !timeStr) return false;
  return parseSlotDateTime(dateStr, timeStr).getTime() >= Date.now() + MIN_APPOINTMENT_LEAD_MS;
}

function filterAllowedSlots(dateStr, slots) {
...
function getFirstBookableDateStr(maxDaysAhead = 90) {
...
module.exports = {
  MIN_APPOINTMENT_LEAD_MS,
  getEarliestAppointmentTime,
  getMinAppointmentDateStr: () => formatDateLocal(getEarliestAppointmentTime()),
  getClinicTimeSlots,
  getFirstBookableDateStr,
  isAppointmentSlotAllowed,
  filterAllowedSlots,
};
